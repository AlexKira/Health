...
def parameters(request):
    ...
    oldkeys = list(result.keys())
    verbose_names = [Result._meta.get_field(key).verbose_name for key in oldkeys]
    help_texts = [Result._meta.get_field(key).help_text for key in oldkeys]
    values = list(result.values())
    # results = dict(zip(oldkeys, dict(zip(verbose_names, values))))
    results = [
        {'name': oldkey, 'verbose_name': verbose_name, 'value': value, 'help_text': help_text}
        for oldkey, verbose_name, value, help_text in zip(oldkeys, verbose_names, values, help_texts)
    ]
